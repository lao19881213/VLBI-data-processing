# Program to select AGNs from astrogeo website.
...
logger = logging.getLogger('select-AGNs')
logfile = "./select_AGNs.log"
...
#First Check the Internet connection with timeout of 10 seconds:
import requests
...
def process_wrapper(lineID):
    outfile1= open("bb.2.tmp", "a+")
    outfile2= open("bb.3.tmp", "a+")
         ...
         for i,line in enumerate(infile):
             if i != lineID:
                ...
                if len(containers)!=0:
                   ...
                   print('%s %s' % (str_id,sub_container1.text), file=outfile1)
                   print('%s %s' % (str_id,sub_container2.text), file=outfile2)
                else:
                   print('%s  11:11:11.1111' % str_id, file=outfile1)
                   print('%s  +11:11:11.111' % str_id, file=outfile2)
...
#init objects, change to total cpu cores of your machine 
pool = mp.Pool(40)
jobs = []
#create jobs
with open("bb.1") as f:
...
list_order=[]
#reorder data
with open("bb.2.tmp", "r") as f:
    for line in f:
        columns = line.split(' ')
        list_bb2.append(columns[2])
        list_order.append(int(columns[0]))

my_bb2 = sorted(zip(list_bb2,list_order), key=lambda x: x[1])

f = open('bb.2.tmp', 'r')
with open("bb.2", "w") as text_file:
    for ib in range(len(my_bb2)):
        print(my_bb2[ib][0], file=text_file)
f.close()

list_bb3=[]
list_order_1=[]
with open("bb.3.tmp", "r") as f:
    for line in f:
        columns = line.split(' ')
        list_bb3.append(columns[2])
        list_order_1.append(int(columns[0]))

my_bb3 = sorted(zip(list_bb3,list_order_1), key=lambda x: x[1])

f = open('bb.3.tmp', 'r')
with open("bb.3", "w") as text_file:
    for ib in range(len(my_bb3)):
        print(my_bb3[ib][0], file=text_file)
...
#Search the sources within 1 arcmin around the obtained coordinates
...
def process_third_step(lineID):
    z_out=open("cc.1.tmp", "a+")
    sep_out=open("cc.2.tmp", "a+")
    with open('bb.6', 'r') as f1, open('bb.7', 'r') as f2:
         for ID, line in enumerate(zip(f1, f2)):
                if ID != lineID:
                   ...
                   x = line[0]
                   y = line[1]
                   my_url = 'http://ned.ipac.caltech.edu/cgi-bin/nph-objsearch?in_csys=Equatorial&in_equinox=J2000.0&lon='+ x[0:(len(x)-1)]+'&lat='+ y[0:(len(y)-1)]+'&radius=1.0&out_csys=Equatorial&out_equinox=J2000.0&obj_sort=Distance+to+search+center&of=pre_text&zv_breaker=30000.0&list_limit=32&img_stamp=YES&z_constraint=Unconstrained&z_value1=&z_value2=&z_unit=z&ot_include=ANY&in_objtypes3=Radio&nmp_op=ANY&search_type=Near+Position+Search'
                   #print(my_url)
                   uClient=uReq(my_url)
                   page_html=uClient.read()
                   uClient.close()
                   ...
                   if len(containers)!=0:
                      ...
                      my_url1 = 'http://ned.ipac.caltech.edu/cgi-bin/nph-objsearch?in_csys=Equatorial&in_equinox=J2000.0&lon='+ x[0:(len(x)-1)]+'&lat='+ y[0:(len(y)-1)]+'&radius=2.0&out_csys=Equatorial&out_equinox=J2000.0&obj_sort=Distance+to+search+center&of=pre_text&zv_breaker=30000.0&list_limit=32&img_stamp=YES&z_constraint=Unconstrained&z_value1=&z_value2=&z_unit=z&ot_include=ANY&in_objtypes3=Radio&nmp_op=ANY&search_type=Near+Position+Search'
                      ...
                      if len(containers1)!=0:
                         ...
                         my_url2 = 'http://ned.ipac.caltech.edu/cgi-bin/nph-objsearch?in_csys=Equatorial&in_equinox=J2000.0&lon=11h11m11.1111&lat=+11d11m11.111&radius=2.0&out_csys=Equatorial&out_equinox=J2000.0&obj_sort=Distance+to+search+center&of=pre_text&zv_breaker=30000.0&list_limit=32&img_stamp=YES&z_constraint=Unconstrained&z_value1=&z_value2=&z_unit=z&ot_include=ANY&in_objtypes3=Radio&nmp_op=ANY&search_type=Near+Position+Search'
                         ...
                         logging.info('No radio source found within 2.0 arcmin radius from NED around coordinates:\n%s %s' % (x,y))
                   #i=i+1
                   ...
                   line = inp[0].split() 
                   if line[0]=='...':
                      z_value='%d NaN' %  lineID  #We can make it '0.0' also.
                   else:
                      z_value='%d %s' % (lineID,line[0])
                   separation='%d %s' % (lineID,line[1])
                   z_out.write(z_value)
                   z_out.write('\n')
                   sep_out.write(separation)
                   sep_out.write('\n')
                   break
    z_out.close()
    sep_out.close()
...
list_order_2=[]
#reorder data
with open("cc.1.tmp", "r") as f:
    for line in f:
        columns = line.split(' ')
        columns[1] = columns[1].split('\n')[0]
        list_cc1.append(columns[1])
        list_order_2.append(int(columns[0]))

my_cc1 = sorted(zip(list_cc1,list_order_2), key=lambda x: x[1])

with open("cc.1", "w") as text_file:
    for ib in range(len(my_cc1)):
        print(my_cc1[ib][0], file=text_file)

list_cc2=[]
list_order_3=[]
with open("cc.2.tmp", "r") as f:
    for line in f:
        columns = line.split(' ')
        columns[1] = columns[1].split('\n')[0]
        list_cc2.append(columns[1])
        list_order_3.append(int(columns[0]))

my_cc2 = sorted(zip(list_cc2,list_order_3), key=lambda x: x[1])

with open("cc.2", "w") as text_file:
    for ib in range(len(my_cc2)):
        print(my_cc2[ib][0], file=text_file)
'''
...
import pandas as pd
files=['aa.4','bb.6','bb.7','cc.1','cc.2']
frames = [ pd.read_csv(x, sep=' ',na_filter=False, header=None) for x in files ]
df = pd.concat(frames,axis=1)
df.to_csv('source_list.dat', sep=' ', header=['Name','RA','DEC','Redshift','Sep_arcmin'], index=False)
